--[[
    PATHOS ULTIMATE - COMPLETE EDITION 2026
    Features:
    ✓ Full Executor Detection (50+ Executors)
    ✓ Key System with Auto-Save
    ✓ Working Features After Key Entry
...
local Utils = {} do
    ...
    function Utils.try(fn, ctx)
        ...
        if not ok then
            warn("[Pathos] " .. ctx .. ": " .. tostring(result))
            return nil
        end
        return result
    end
...
function KeySystem:create()
    ...
    self.key = Utils.readFile(CONFIG.FOLDER_KEYS .. "/saved.key")
    
    local gui = Instance.new("ScreenGui")
    gui.Name = "PathosKeySystem"
    ...
    frame.Size = UDim2.new(0, 400 * scale, 0, 300 * scale)
    frame.Position = UDim2.new(0.5, -200 * scale, 0.5, -150 * scale)
    ...
    execLabel.Parent = frame
    
    -- Key input
    local inputFrame = Instance.new("Frame")
    inputFrame.Size = UDim2.new(1, -40 * scale, 0, 40 * scale)
    inputFrame.Position = UDim2.new(0, 20 * scale, 0, 80 * scale)
    ...
    keyInput.Size = UDim2.new(1, -20 * scale, 1, 0)
    keyInput.Position = UDim2.new(0, 10 * scale, 0, 0)
    keyInput.BackgroundTransparency = 1
    keyInput.PlaceholderText = "Enter Your Key"
    keyInput.PlaceholderColor3 = Color3.fromRGB(150, 150, 150)
    keyInput.TextColor3 = Color3.fromRGB(255, 255, 255)
    keyInput.Text = self.key or ""
    keyInput.TextSize = 14 * scale
    ...
    submitBtn.Size = UDim2.new(0.45, 0, 0, 40 * scale)
    submitBtn.Position = UDim2.new(0.05, 0, 0, 130 * scale)
    submitBtn.BackgroundColor3 = CONFIG.COLORS.PRIMARY
    ...
    submitCorner.CornerRadius = UDim.new(0, 8 * scale)
    submitCorner.Parent = submitBtn
    
    local getKeyBtn = Instance.new("TextButton")
    getKeyBtn.Size = UDim2.new(0.45, 0, 0, 40 * scale)
    getKeyBtn.Position = UDim2.new(0.5, 0, 0, 130 * scale)
    getKeyBtn.BackgroundColor3 = Color3.fromRGB(60, 60, 65)
    getKeyBtn.Text = "GET KEY"
    getKeyBtn.TextColor3 = Color3.fromRGB(200, 200, 200)
    getKeyBtn.TextSize = 14 * scale
    ...
    discordBtn.Position = UDim2.new(0, 20 * scale, 0, 180 * scale)
    ...
    status.Position = UDim2.new(0, 20 * scale, 0, 220 * scale)
    ...
    submitBtn.MouseButton1Click:Connect(function()
        local key = Utils.sanitize(keyInput.Text)
        if key and key ~= "" then
            ...
            Utils.writeFile(CONFIG.FOLDER_KEYS .. "/saved.key", key)
            ...
            -- Load main script
            task.spawn(function()
                local content = Utils.request(CONFIG.API_URL)
                if content then
    ...
    getKeyBtn.MouseButton1Click:Connect(function()
        Utils.setClipboard(CONFIG.KEY_PROVIDERS.LINKVERTISE)
...
pcall(function()
    ...
    if queue then
        queue([[loadstring(game:HttpGet("https://raw.githubusercontent.com/th3-osc/Pathos/main/Pathos%20Ultimate.lua"))()]])
    end
end)

print("✅ Pathos Ultimate initialized!")
print("⚡ Detected: " .. Executor.name)
print("🔑 Enter your key to activate features")